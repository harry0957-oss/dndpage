...
async function loadData() {
  const endpoints = ['config', 'names', 'professions', 'traits', 'towns'];
  const responses = await Promise.all(
    endpoints.map((key) => fetch(`/data/${key}.json`).then((res) => res.json()))
  );

  store.config = responses[0];
  store.names = responses[1];
  store.professions = responses[2];
  store.traits = responses[3];
  store.towns = responses[4];
}
